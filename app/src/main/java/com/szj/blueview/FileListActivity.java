...
public class FileListActivity extends Activity {// 创建Activity
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_filelist); // 设置layout
        final ListView lv = (ListView) this.findViewById(R.id.lv_filelist); // 获取ListView
        Button back = (Button) this.findViewById(R.id.back_filelist); // 获取返回按钮
        final File[] files = getFiles(rootPath); // 调用getFiles方法获取根目录下文件列表
        currDirTV = (TextView) this.findViewById(R.id.currDirTV_filelist); // 获取ListView
        currPath = rootPath;
                ...
                BaseAdapter ba = new BaseAdapter()// 创建适配器
                {
                    ...
                    public View getView(int arg0, View arg1, ViewGroup arg2) {
                        LinearLayout ll = new LinearLayout(
                                FileListActivity.this); // 创建LinearLayout
                        ll.setOrientation(LinearLayout.VERTICAL); // 竖直排列
                        ll.setPadding(5, 5, 5, 5); // 设置留白
                        TextView tv = new TextView(FileListActivity.this); // 创建TextView
